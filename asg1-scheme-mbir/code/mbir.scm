    ...
    (if (hash-has-key? *function-table* (car expr))
        ...
        (if (hash-has-key? *array-table* (cadr expr))
            ...
            (vector-ref (hash-ref *array-table* (cadr expr)) 
              (- (exact-round(eval-expr (caddr expr))) 1))
                (die '(Invalid expression)))))))

;;simplify
(define (interp-dim args continuation)
    ;(printf "interp-dim: expr: ~a ~n" (car args))
    ;(printf "interp-dim: expr: ~a ~n" (cadr (car args)))
    ;(printf "interp-dim: expr: ~a ~n" (caddr (car args)))
    (hash-set! *array-table* (cadr (car args)) 
    (make-vector (abs (exact-round (eval-expr (caddr (car args)))))))
...
(define (interp-let args continuation)
    (if (pair? (car args))
        (begin
            ;(printf "interp-let: expr: ~a ~n" (caddr (car args)))
            ;(printf "interp-let: expr: ~a ~n" (cadar args))
            ;(printf "interp-let: expr: ~a ~n" (cadr args))
